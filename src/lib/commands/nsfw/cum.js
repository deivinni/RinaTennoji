const { Embed } = require('../../../util/functions/index'),
  { cumArts, cumsluts } = new (require('nekos.life'))().nsfw;

module.exports = {
  exec: async (msg) => {
    msg.channel.startTyping(true);
    const embed = new Embed(msg.author, 'Nekos.life');
    return await (msg.args[0] === '--art' ? cumArts() : cumsluts()).then(async(r) => {
          ...
          await collector.on('collect', async(r) => {
            switch (r.emoji.id) {
              case msg.emoji.ids.normais.bot.reload:
                await (msg.args[0] === '--art' ? cumArts() : cumsluts()).then(async(r1) => {
