const { Embed } = require('../../../util/functions/index'),
  { neko, nekoGif, eroNeko } = new (require('nekos.life'))().nsfw;

module.exports = {
  exec: async (msg) => {
    msg.channel.startTyping(true);
    const embed = new Embed(msg.author, 'Nekos.life');
    return await (msg.args[0] === '--gif' ? nekoGif() : msg.args[0] === '--ero' ? eroNeko() : neko()).then(async(r) => {
          ...
          await collector.on('collect', async(r) => {
            switch (r.emoji.id) {
              case msg.emoji.ids.normais.bot.reload:
                await (msg.args[0] === '--gif' ? nekoGif() : msg.args[0] === '--ero' ? eroNeko() : neko()).then(async(r1) => {
