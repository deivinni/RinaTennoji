const { Embed } = require('../../../util/functions/index'),
  { pussy, pussyArt, pussyGif, pussyWankGif } = new (require('nekos.life'))().nsfw;

module.exports = {
  exec: async (msg) => {
    msg.channel.startTyping(true);
    const embed = new Embed(msg.author, 'Nekos.life');
    return await (msg.args[0] === '--gif' ? pussyGif() : msg.args[0] === '--art' ? pussyArt() : msg.args[0] === '--wank' ? pussyWankGif() : pussy()).then(async(r) => {
          ...
          await collector.on('collect', async(r) => {
            switch (r.emoji.id) {
              case msg.emoji.ids.normais.bot.reload:
                await (msg.args[0] === '--gif' ? pussyGif() : msg.args[0] === '--art' ? pussyArt() : msg.args[0] === '--wank' ? pussyWankGif() : pussy()).then(async(r1) => {
